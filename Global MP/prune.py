...
model_names = ['resnet50_imagenet', 'mobilenet_v2', 'mobilenet_v2_cifar10', 'efficientnet-b2', 'lenet5caffe', 'wrn-22-8', 'wrn-16-8', 'wrn-16-10']
...
parser.add_argument('--weight-decay', type=float, default=0.000007)
...
def main_worker(gpu, ngpus_per_node, args):
	...
	df = pd.DataFrame(columns=columns)
	log_path = "./run12_log_Epochs="+str(args.epochs)+",LR="+str(args.lr)+",Batch="+str(args.batch_size)+".csv"
...
def save_checkpoint(state, is_best, args):
	if is_best:
		...
		best_model_name = "run12_BestModel_Epoch{}.pth.tar".format(state['epoch'])
